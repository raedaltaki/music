...
export default {
  ...
  data() {
    return {
      ...
      comment_alert_message: 'Pease wait! Your comment is being submitted',
      comments: [],
      sort: '1',
    };
  },
  computed: {
    ...
    sortedComments() {
      return this.comments.slice().sort((a, b) => {
        if (this.sort === '1') {
          return new Date(b.datePosted) - new Date(a.datePosted); // sort latest to oldest
        }

        return new Date(a.datePosted) - new Date(b.datePosted); // sort oldest to latest
      });
    },
  },
  async created() {
  ...
  methods: {
    ...
    async addComment(values, { resetForm }) {
      ...
      this.comment_alert_message = 'Pease wait! Your comment is being submitted';
